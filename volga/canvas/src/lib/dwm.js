...
assert(INITIAL_MEMORY == 83886080, 'Detected runtime INITIAL_MEMORY setting.  Use -s IMPORTED_MEMORY to define wasmMemory dynamically');
...
var ASM_CONSTS = {
  35996: function($0, $1) {try { var fs = UTF8ToString($0); var ems = UTF8ToString($1); FS.mkdir(ems); FS.mount(NODEFS, {root: fs}, ems); } catch (error) { if (error.code !== 'EBUSY') { console.error(error); } }}
};
