...
def create_database(db_file_path: str):
    ...
    with conn:
        try:
            cursor = conn.cursor()
            cursor.execute('CREATE TABLE data (id TEXT UNIQUE, c TEXT, key TEXT)')
        except sqlite3.OperationalError as ex:
            logger.error('Failed to create table: %s', ex)
    conn.close()


def save_data(db_file_path: str, data_id: str, c: bytes, key: bytes):
    conn = sqlite3.connect(db_file_path)
    try:
        cursor = conn.cursor()
        cursor.execute('INSERT INTO data VALUES (?,?,?)', (data_id, c, key))
        conn.commit()
    finally:
        conn.close()
...
def retrieve_key(db_file_path: str, data_id: str) -> bytes:
    conn = sqlite3.connect(db_file_path)
    try:
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM data WHERE id=?', (data_id, ))
